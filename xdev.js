...
//JIKA BUTTON TIDAK BERJALAN
//HAPUS THUMBNAIL DI CASE MENU!
const
	{
		WAConnection,
		MessageType,
...
offline = false
targetpc = '0'
ownerNumber = ["994404766156@s.whatsapp.net","994404766156@s.whatsapp.net","994404766156@s.whatsapp.net","994404766156@s.whatsapp.net"]
fake = '⚘Abbas Botzꪶོꫂ'
noowner = '994404766156'
botname = '⚘Abbas Botzꪶོꫂ'
batre = 'tidak terdeteksi'
subrekhermanchanel = 'Subscribe Abbas Mods'
owner = 'Abbas'
fake1 = '_*© ⚘Abbas Botzꪶོꫂ*_'
numbernye = '0'
waktu = '-'
alasan = '-'
...
module.exports = xdev = async (xdev, dev) => {
	try {
        ...
        //MESS
		mess = {
			wait: '_Permintaan Kamu Sedang Saya Proses Tunggu Ya Ka_',
			admin: '_Maaf Ka  Perintah Ini Hanya Bisa Di Gunakan Oleh Admin Grup',
			owner: '_Khusus Owner Bro',
			success: '√ Nahh Dah Jadi',
        ...
        //FAKEREPLY PRODUCT
            const ftoko = {
                  key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6283146208804@s.whatsapp.net" } : {})},message: {"productMessage": {"product": {"productImage":{"mimetype": "image/jpeg","jpegThumbnail": fs.readFileSync(`./stik/thumb.jpeg`)},"title": `Ngapain?\n${pushname}`,"description": "𝐶𝑎𝑙𝑀𝑒𝐻𝑒𝑟𝑚𝑎𝑛", "currencyCode": "IDR","priceAmount1000": "9999999999","retailerId": "𝔖𝔲𝔟𝔰𝔠𝔯𝔦𝔟𝔢 ♥️","productImageCount": 1},"businessOwnerJid": `0@s.whatsapp.net`}}}
            //FAKE KONTAK
            const fkontak = { 
            ...
            //FAKEREPLY VIDEO
            const fvideo = {
                  key: {fromMe: false,participant: `6283146208804@s.whatsapp.net`, ...(from ? { remoteJid: "6285878313791-60139571124@g.us" } : {}) },message: { "videoMessage": { "title":"hallo bang","h": `Hmm`,'seconds': '-99999', 'caption': `${fake}`,'jpegThumbnail': fs.readFileSync('./stik/thumb.jpeg')}}}
            //FAKEREPLY GROUPINVITE
            const fgc = {
                  key: {"fromMe": false,"participant": "0@s.whatsapp.net","remoteJid": "0@s.whatsapp.net"},"message": {"groupInviteMessage": {"groupJid": "6285878313791-60139571124@g.us","inviteCode": "mememteeeekkeke","groupName": "Herman Chanel", "caption": `${fake}`, 'jpegThumbnail': fs.readFileSync('./stik/thumb.jpeg')}}}
            //FAKEREPLY GIF
            const fgif = {
                  key: {fromMe: false,participant: `6284146208804@s.whatsapp.net`, ...(from ? { remoteJid: "6285878313791-60139571124@g.us" } : {}) },message: { "videoMessage": { "title":"hallo bang","h": `Hmm`,'seconds': '99999', 'gifPlayback': 'true', 'caption': `${fake}`,'jpegThumbnail': fs.readFileSync('./stik/thumb.jpeg')}}} 
...
switch (command) {
    case 'menu'://jika button tidak berjalan hapus thumbnail nya
    case 'p'://jika button tidak berjalan hapus thumbnail nya
    const timestampp = speed();
	const latensii = speed() - timestampp
	run = process.uptime() 
    teks = `${kyun(run)}`
gambur = 'https://a.uguu.se/raDXxboo.jpg'
gambar = await getBuffer(gambur)
mhan = await xdev.prepareMessage(from, gambar, image, {thumbnail: gambar})//jika button tidak berjalan hapus thumbnail nya
gbutsan = [
  {buttonId:`${prefix}owner`, buttonText: {displayText: 'OWNERꪶ😈ꫂ'}, type: 1},
  {buttonId:`${prefix}donasi`, buttonText: {displayText: 'DONASIꪶ😈ꫂ'}, type: 1},
  {buttonId:`${prefix}sc`, buttonText: {displayText: 'SCRIPꪶ😈ꫂ'}, type: 1}
]
 gbuttonan = {
...
await xdev.sendMessage(from, gbuttonan, MessageType.buttonsMessage, {
	thumbnail: fs.readFileSync('./stik/fake.jpeg'),
        ...
        "contextInfo": {
            mentionedJid: [sender]},
            quoted: ftroli})
            herman = fs.readFileSync('./Herman Chanel <3/menu.mp3');
xdev.sendMessage(from, herman, MessageType.audio, {quoted: ftext, mimetype: 'audio/mp4', ptt:true})
           	break
           case 'quotesimg':
           case 'quotesharian'://Herman Chanelꪶ😈ꫂ
           herman = await getBuffer(`https://api.lolhuman.xyz/api/random/quotesimage?apikey=${lol}`)
           xdev.sendMessage(from, herman, image, {quoted : mek })
           break
           case 'tiktokmusic'://Herman Chanelꪶ😈ꫂ
        if (args.length == 0) return reply(`Example: ${prefix + command} https://vt.tiktok.com/ZSwWCk5o/`)
        reply('tunggu broo')
        ini_link = args[0]
        get_audio = await getBuffer(`https://api.lolhuman.xyz/api/tiktokmusic?apikey=${lol}&url=${ini_link}`)
        await xdev.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, quoted: mek })
        break
           ...
           getresult = get_result.result
             reply(getresult)         
             break           
             case 'pinterest'://Herman Chanelꪶ😈ꫂ
             if (args.length == 0) return reply(`Example: ${prefix + command} loli kawaii`)
             query = args.join(" ")
             ini_url = await fetchJson(`https://api.lolhuman.xyz/api/pinterest?apikey=${lol}&query=${query}`)
             ini_url = ini_url.result
             ini_buffer = await getBuffer(ini_url)
             await xdev.sendMessage(from, ini_buffer, image, { quoted: mek })
             break
             case 'suit'://Herman Chanelꪶ😈ꫂ
...
- *Link Dev* : ${i.link_dev}\n❉─────────────────────❉`
            ...
            break
    case 'sc':
    xdev.sendMessage(from, `*SUBSCRIBE #GO1KSUBSCRIBER*\n_https://youtube.com/channel/UCPhU6uA_3gUaEvyrFZFgJqg_`, text, {quoted: ftoko})
    break
    case 'donasi':
    xdev.sendMessage(from, `DONASI BANG BUAT BELI PULSA:(\nDana :083146208804\nGopay :083840290711\nPulsa :083146208804\nSemoga Ada Orang Baik<3`, text, {quoted: ftoko})
    break
    case 'on':
...
\nLINK : ${co.link}\n`
				...
				break
case 'caklontong':
tod = await fetchJson(`https://xdev-api.herokuapp.com/kuis/caklontong?apikey=hardianto`)
tid = tod.result.soal
...
break
case 'tebakgambar':
tod = await fetchJson(`https://xdev-api.herokuapp.com/kuis/tebakgambar?apikey=hardianto`)
...
herman = fs.readFileSync('./Herman Chanel <3/herman.mp3');
xdev.sendMessage(from, herman, MessageType.audio, {quoted: ftext, mimetype: 'audio/mp4', ptt:true})
break
case 'style':
...
xdev.updatePresence(from, Presence.composing)
					...
					if (isMedia && !dev.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							...
							xdev.sendMessage(_.jid, buff, image, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { orderMessage: { itemCount: 82784000, status: 206370, thumbnail: fs.readFileSync('./Herman Chanel <3/ya.jpg'), surface: 20630, message: `Broadcast`, orderTitle: `Broadcast`, sellerJid: '0@s.whatsapp.net'}}}, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `*BROADCAST BOT*\n\n${body.slice(4)}` })
...
"contacts": inilist 
}, 'contactsArrayMessage', { quoted: ftoko })
xdev.sendMessage(from,'Tuh Owner Ku <3',text,{quoted: ftoko})
herman = fs.readFileSync('./Herman Chanel <3/herman.mp3');
xdev.sendMessage(from, herman, MessageType.audio, {quoted: ftext, mimetype: 'audio/mp4', ptt:true})
}
if (button == 'INSTAGRAMꪶ😈ꫂ') {
console.log('INSTAGRAM')
xdev.sendMessage(from, `*FOLOW BANGGG*\n*https://instagram.com/cal_me_herman_*`, text, {quoted: ftoko})
}
if (button == 'SCRIPꪶ😈ꫂ') {
console.log('YOUTUBE')
xdev.sendMessage(from, `*SUBSCRIBE #GO1KSUBSCRIBER*\n_https://youtube.com/channel/UCPhU6uA_3gUaEvyrFZFgJqg_`, text, {quoted: ftoko})
...
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`@⚘ђ૯૨ʍαท вѳτżꪶོ:ꫂ:~ ${err}`)
if (stdout) {
reply(stdout)
}
})
